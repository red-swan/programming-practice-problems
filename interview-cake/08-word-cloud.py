...
def make_word_cloud(input_string):
    ...
    word_end_index = None
    for running_index in range(len(input_string)):
        if word_start_index is None and input_string[running_index].lower() in alphabet:
            word_start_index = running_index

        if word_start_index is not None:
            if input_string[running_index].lower() not in alphabet.union({'-'}):
               word_end_index = running_index
            elif running_index == len(input_string) - 1 and \
                 input_string[running_index].lower() in alphabet:
                     word_end_index = running_index + 1

        if word_start_index is not None and word_end_index is not None:
            current_word = ''.join(input_string[word_start_index:word_end_index]).lower()
...
str1 = 'After beating the eggs, Dana read the next step:' 
str2 = 'Add milk and eggs, then add flour and sugar. Hot-cold'
make_word_cloud(str1 + str2)
sample2 = "We came, we saw, we conquered...then we ate Bill's (Mille-Feuille) cake."
sample3 = 'The bill came to five dollars.'
make_word_cloud(sample2 + sample3)
...
a = WordCloudData(sample2 + sample3)
a.words_to_counts